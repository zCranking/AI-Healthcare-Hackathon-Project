...
from .auditor import Finding, _format_fhir
...
def _one_vote(
    ...
    system = (
        ...
        "an automated note auditor. You have THREE sources: the visit transcript, the patient's "
        "structured FHIR chart data, and (when present) the patient-facing after-visit summary. "
        "The FHIR chart records the patient's ACTUAL data and is authoritative - if the note "
        "misstates something the chart records correctly, the note is wrong, not the chart. A "
        "claim is adequately supported if EITHER the transcript OR the chart grounds it. Judge "
        "only the flag in front of you, against all sources. Be decisive."
    )
    user = (
        f"SOURCE TRANSCRIPT:\n{transcript}\n\n"
