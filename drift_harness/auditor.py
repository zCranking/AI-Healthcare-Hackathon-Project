...
_SYSTEM = (
    ...
    "Pay special attention to NEGATIVE and absolute assertions in the note ('denies X', 'no Y', "
    "'reports no Z', 'never', 'no history of'). For each, find the patient's ACTUAL words in the "
    "transcript. If the patient hedged or was uncertain ('I don't think so', 'not really', "
    "'sometimes but not bad', 'I don't look that closely'), then a clean 'denies'/'no' is a "
    "CONTRADICTION (refuted by the transcript) - the note hardened an uncertain answer into a "
    "definitive one. Flag it; do not mark it supported. Likewise scan every symptom the patient "
    "raised even briefly (then dropped) and every medication the patient said they STOPPED - these "
    "are the highest-yield drift sites.\n\n"
